...
@interface ZBWaterView()<YNFlowViewDelegate>
{
...
- (id)dequeueReusableCellWithIdentifier:(NSString *)identifier
{
    ...
    {
        YNFlowView *flowView = [cellsWithIndentifier lastObject];
 ...
 *  @param flowView 视图
 */
- (void)recycleFlowViewIntoReusableQueue:(YNFlowView *)flowView
{
...
- (void)waterScroll:(UIScrollView *)scrollView
{
    ...
    if (_visibleRange.length > 0) {
        ...
        CGFloat topOutHeight = (topOutPoint).y+[[_resHeightDic objectForKey:[NSNumber numberWithInteger:_visibleRange.location]] floatValue];
        if (topOutHeight < TopLineHeight) {
            YNFlowView *flowView = [_visibleViews firstObject];
    ...
    if ((_visibleRange.location+_visibleRange.length <= _bottomDrawPointVector.size())
        ...
        if (bottomOutHeight>BottomLineHeight) {
            YNFlowView *flowView = [_visibleViews lastObject];
            [self recycleFlowViewIntoReusableQueue:flowView];
            [flowView removeFromSuperview];
            [_visibleViews removeObject:flowView];
            _visibleRange.length--;
 ...
 *  @param resArr 视图数组
 */
- (UIView *)drawView:(CGPoint)drawPoint index:(NSUInteger)index
{
    CGRect flowViewRect;
    flowViewRect.origin = drawPoint;
    flowViewRect.size = CGSizeMake(_resWidth, [[_resHeightDic objectForKey:[NSNumber numberWithInteger:index]] floatValue]);
    YNFlowView *view = [self.waterDataSource waterView:self flowViewAtIndex:index];
...
#pragma mark - YNFlowViewDelegate
